...
import com.example.bnilist.holder.RegionHolder;
...
public class RegionAdapter extends RecyclerView.Adapter<RegionHolder> {
    ...
    Context ctx;
    public ArrayList<RegionModel> regions;

    public RegionAdapter(Context ctx, ArrayList<RegionModel> regions){
        this.ctx = ctx;
        this.regions= regions;
    }
    @Override
    public RegionHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.activity_wilayah_list_kantor, null);
        RegionHolder holder = new RegionHolder(v);
        return holder;
    }
    ...
    public void onBindViewHolder(RegionHolder holder, int position) {
        //BIND DATA
        holder.tvCode.setText(regions.get(position).getCode());
